...
export class ChatController {
  ...
  async create(
    ...
    let created = await this.chatRepository.findOne({where: {users: chat.users as PredicateComparison<string[]>}})
    if (created !== null) {
      return created;
    }
    created = await this.chatRepository.findOne({where: {users: chat.users?.reverse() as PredicateComparison<string[]>}})
    if (created !== null) {
      return created;
    }
